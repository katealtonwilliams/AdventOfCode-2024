...
def read_in_robots(input_file: str) -> list[dict[str:tuple[int]]]:
    ...
    with open(input_file) as raw_robots:
        for robot in raw_robots.readlines():
            ...
            position_and_velocity['position'] = tuple(map(int, raw_position_and_velocity[0]))
            position_and_velocity['velocity'] = tuple(map(int, raw_position_and_velocity[1]))
            all_robots.append(position_and_velocity)
    return all_robots

def get_final_position(robot: dict[str:tuple[int]], width: int, height: int, seconds: int) -> tuple[int]:
...
def get_quadrant(final_x: int, final_y: int, middle_x: int, middle_y: int) -> int|None:
    if final_x < middle_x:
        if final_y < middle_y:
            return 1
        if final_y > middle_y:
...
def calculate_safety_factor_p1(input_file: str, width: int = 101, height: int = 103, seconds: int = 100) -> int:
...
def plot_grid_for_one_time(robots: dict[str:tuple[int]], seconds: int, width: int = 101, height: int = 103) -> np.ndarray:
