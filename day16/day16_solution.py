...
def find_shortest_path(input_file: str) -> int:
    ...
    while all_paths:
        ...
        if maze[new_pos_straight] == "E":
            return score + 1
        if (
            maze[new_pos_straight] != "#"
            and (new_pos_straight, direction) not in seen_paths
        ):
        ...
        for perp_direction in DIR_MAP[direction]:
            new_pos_perp = (
                position[0] + perp_direction[0],
                position[1] + perp_direction[1],
            )
            if maze[new_pos_perp] == "E":
                return score + 1001
            if (
                maze[new_pos_perp] != "#"
                and (new_pos_perp, perp_direction) not in seen_paths
            ):
...
def find_shortest_scores(maze: np.ndarray, max_score: int) -> int:
    ...
    while all_paths:
        ...
        shortest_scores.setdefault(position, []).append(
            (score, current_direction, turned)
        )
        # create function so not doubling effort
        new_pos_straight = (
        ...
        if maze[new_pos_straight] == "E":
            ...
            shortest_scores.setdefault(new_pos_straight, []).append(
                (score + 1, current_direction, False)
            )
            continue
        if (
            maze[new_pos_straight] != "#"
            and (score + 1, new_pos_straight, current_direction, False)
            not in seen_paths
        ):
            heapq.heappush(
                all_paths, (score + 1, new_pos_straight, current_direction, False)
            )
            seen_paths.add((score + 1, new_pos_straight, current_direction, False))
        for perp_direction in DIR_MAP[current_direction]:
            ...
            if maze[new_pos_perp] == "E":
                ...
                shortest_scores.setdefault(new_pos_perp, []).append(
                    (score + 1001, perp_direction, True)
                )
                continue
            if (
                maze[new_pos_perp] != "#"
                and (score + 1001, new_pos_perp, perp_direction, True) not in seen_paths
            ):
                heapq.heappush(
                    all_paths, (score + 1001, new_pos_perp, perp_direction, True)
                )
                seen_paths.add((score + 1001, new_pos_perp, perp_direction, True))


def plot_scores(
    maze_shape: tuple[int], scores: list[tuple[int, tuple[int]]], max_score: int
) -> np.ndarray:
...
def find_all_points_on_shortest_path(input_file: str) -> int:
    ...
    final_points = set()
    all_paths = [
        (score, end_point, direction, turned)
        for score, direction, turned in point_to_scores[end_point]
    ]
    heapq.heapify(all_paths)
    while all_paths:
        ...
        if (
            ...
            direction,
            turned,
        ) not in seen_paths and new_pos in point_to_scores:
