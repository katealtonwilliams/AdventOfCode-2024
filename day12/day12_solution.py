            ...
            if (coord := (row_id, col_id)) not in seen_plants:
                ...
                while growing_region:
                    ...
                    for orthogonal_plant in get_orthogonal_plants(current_coord):
                        if point_is_in_grid(orthogonal_plant, farm) and farm[orthogonal_plant] == plant and orthogonal_plant not in seen_plants:
...
def traverse_perimiter(perimeter_points: set[tuple[int]]) -> None:
    sub_region_borders = []
...
def get_number_of_sides(region_number: set[tuple[int]], big_grid: np.ndarray) -> int:
    ...
    for row, column in zip(region_rows, region_columns):
        all_perimeter_walls = all_perimeter_walls.union(get_perimeter_walls(region_number, (row, column), big_grid))
